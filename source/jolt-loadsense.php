...
add_action('admin_enqueue_scripts', function($hook) {
    if ($hook !== 'toplevel_page_jolt-loadsense') return;

    wp_enqueue_media();
    wp_enqueue_style('wp-color-picker');
    wp_enqueue_script(
        'jolt-loadsense-admin',
...
add_action('admin_init', function() {
    ...
    register_setting('jolt_loadsense_settings', 'jolt_loadsense_background_color');
});
...
function jolt_loadsense_settings_page() {
    ...
    $background_color = get_option('jolt_loadsense_background_color', '#ffffff');
    ?>
    <div class="wrap">
        <?php
        ...
        ?>
        <h1>JOLT LoadSense v<?php echo esc_html($version); ?></h1>
        <div style="margin-top: 2em; background: #fffbe5; border: 1px solid #ffcc00; padding: 1em;">
            <strong>Fake Mode</strong>: This is a fake preloader to test your preloader with and is shown for 3 seconds.<br>
            <strong>Real Mode</strong>: It will disappear only after all content (including images, fonts, and scripts) is fully loaded.
            <br><br>
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                    ...
                    <td>
                        ...
                        <div id="jolt-preview-container" style="margin-top: 1em;">
                            ...
                            <div class="jolt-preview jolt-custom-image" data-animation="custom_image" style="display: none; margin-top: 10px;">
                                <?php if ($custom_image): ?>
                                    <div><img src="<?php echo esc_url($custom_image); ?>" alt="Custom Loader" style="max-height: 150px;"></div>
                                <?php else: ?>
                ...
                <?php if ($custom_image && $animation === 'custom_image'): ?>
                    <tr valign="top">
                        <th scope="row">Custom Image Preview</th>
                        <td>
                            <div style="margin-top: 10px;">
                                <strong>Preview:</strong><br>
                                <img src="<?php echo esc_url($custom_image); ?>" alt="Preloader" style="max-width: 200px; height: auto;">
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
                <tr valign="top">
                    ...
                    <td>
                        ...
                        <p class="description">Upload an image or select one from the media library.<br> If you need a loader image you can pick one from my little <a href="https://github.com/johnoltmans/JOLT-LoadSense/wiki/Loading-gifs" target="_blank">pre-loader library</a>.</p>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row">Preloader Background Color</th>
                    <td>
                        <input type="text" name="jolt_loadsense_background_color" id="jolt_background_color" value="<?php echo esc_attr($background_color); ?>" class="jolt-color-picker" />
                        <p class="description">Choose a background color for the preloader. This is especially useful for custom images without a transparent background.</p>
                    </td>
                </tr>
                <tr valign="top">
    ...
    jQuery(document).ready(function($){
        ...
        $('#jolt_background_color').wpColorPicker();
        ...
        $('#jolt_custom_image_button').on('click', function(e) {
            ...
            frame.on('select', function() {
                ...
                // Reload the page to update the preview
...
add_action('wp_body_open', function() {
    ...
    $background_color = get_option('jolt_loadsense_background_color', '#ffffff');
    ?>
    <div id="jolt-preloader" style="background-color: <?php echo esc_attr($background_color); ?>;">
        <?php 
        ...
        <?php elseif ($animation === 'custom_image' && $custom_image) : ?>
            <img src="<?php echo esc_url($custom_image); ?>" alt="Custom Loader" style="max-width: 250px; max-height: 250px;">
